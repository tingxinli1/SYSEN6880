...
from transformers import AutoTokenizer, AutoModel, AutoConfig, BertConfig, BertTokenizer, BertModel
...
class PreRankingModel(nn.Module):
    """Double BERT model definition"""
    def __init__(self, backbone_dir=backbone_dir, objective='reg'):
        super(PreRankingModel, self).__init__()
        self.objective = objective
        self.encoder_config = BertConfig.from_pretrained(backbone_dir, local_files_only=True)
        self.encoder1 = BertModel.from_pretrained(backbone_dir, local_files_only=True)
        self.encoder2 = BertModel.from_pretrained(backbone_dir, local_files_only=True)
        self.dropout = nn.Dropout(0.1)
        self.dense = nn.Linear(self.encoder_config.hidden_size * 3, 1)
    def forward(self, input_ids1, token_type_ids1, attention_mask1, input_ids2, token_type_ids2, attention_mask2, labels=None):
        sent_emb1 = self.encoder1(input_ids=input_ids1, token_type_ids=token_type_ids1, attention_mask=attention_mask1).pooler_output
        sent_emb2 = self.encoder2(input_ids=input_ids2, token_type_ids=token_type_ids2, attention_mask=attention_mask2).pooler_output
        if self.objective == 'cls':
            crossed_feature = torch.cat([sent_emb1, sent_emb2, torch.abs(sent_emb1 - sent_emb2)], axis=1)
            outputs = self.dropout(crossed_feature)
            ...
            outputs = torch.cosine_similarity(sent_emb1, sent_emb2)
            if labels != None:
                loss_fct = MSELoss()
                loss = loss_fct(outputs, labels)
                outputs = (loss, outputs)
...
def encode_text(text):
    ...
    encoded_input = tokenizer(text, max_length=100, padding='max_length', truncation=True, return_tensors='pt')
...
def load_data(data, batch_size=32, shuffle=False, gold=True):
    """load data"""
    text1, text2, labels = data.text1.tolist(), data.text2.tolist(), data.label.tolist()
    dataset = encode_text(text1) + encode_text(text2)
    if gold:
        dataset += (torch.tensor(labels, dtype=torch.float),)
...
if __name__ == "__main__":
    ...
    # initialize model
    model = PreRankingModel(objective='reg').cuda()
    ...
    # warmup_steps = int(len(dataloader_train) / 10)
    warmup_steps = 100
    ...
    for epoch in range(epochs):
        ...
        if cur_score >= best_score:
            ...
            torch.save(model.state_dict(), '%s_preranking_model.bin' % search_backbone)
